<aside id="sidebar" class="fixed left-0 top-0 h-screen w-[260px] flex flex-col bg-white border-r border-slate-200 z-40 transition-transform duration-300 -translate-x-full lg:translate-x-0">
    <div class="p-6 flex flex-col gap-1 border-b border-slate-100">
        ...
        <span class="text-[12px] font-medium text-slate-500 uppercase tracking-widest">Espace Enseignant</span>
    </div>

    <nav class="flex-1 px-4 py-4 flex flex-col gap-1 overflow-y-auto">
        <x-nav-link-school :active="request()->routeIs('enseignant.dashboard')" icon="dashboard" :href="route('enseignant.dashboard')">
            Tableau de bord
        </x-nav-link-school>
        <x-nav-link-school :active="request()->routeIs('enseignant.classes') || request()->routeIs('enseignant.grades')" icon="edit_note" :href="route('enseignant.classes')">
            Saisie des notes
        </x-nav-link-school>
    </nav>
