<aside id="sidebar" class="fixed left-0 top-0 h-screen w-[260px] flex flex-col bg-white border-r border-slate-200 z-40 transition-transform duration-300 -translate-x-full lg:translate-x-0">
    <div class="p-6 flex flex-col gap-1 border-b border-slate-100">
        ...
        <span class="text-[12px] font-medium text-slate-500 uppercase tracking-widest">Espace Élève</span>
    </div>

    <nav class="flex-1 px-4 py-4 flex flex-col gap-1 overflow-y-auto">
        <x-nav-link-school :active="request()->routeIs('eleve.dashboard')" icon="grade" :href="route('eleve.dashboard')">
            Mes Notes
        </x-nav-link-school>

        <x-nav-link-school :active="request()->routeIs('eleve.profile')" icon="person" :href="route('eleve.profile')">
            Mon Profil
        </x-nav-link-school>
    </nav>
